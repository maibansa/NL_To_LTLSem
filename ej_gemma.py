...
def get_relevant_context(text: str, n_results: int = 7) -> str:
    ...
    try:
        ...
        for meta in results['metadatas'][0]:
            context_lines.append(f"- {meta['concept']} (Type: {meta['type']}) | Definition: {meta.get('original_desc', '')}")
...
def build_compiler_prompt(text: str, context: str, ltl_onto: Dict) -> str:
    # Mantenemos tu prompt exactamente como lo tenías
    return f"""
...
if __name__ == "__main__":
    ...
    for i, rule in enumerate(TEST_RULES, 1):
        ...
        # Extraemos palabras de más de 3 letras de la regla para buscarlas una a una
        words = re.findall(r'\b\w{4,}\b', rule) 
        all_segments = []
        for word in words:
            # Buscamos 3 resultados por cada término para asegurar diversidad (Actores, Actividades, etc)
            seg = get_relevant_context(word, n_results=3)
            if seg: 
                all_segments.append(seg)
        
        # Unimos los resultados y eliminamos líneas duplicadas si las hubiera
