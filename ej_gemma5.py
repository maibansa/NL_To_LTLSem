...
def build_compiler_prompt(text: str) -> str:
...
def call_gemma(prompt: str) -> Optional[ClinicalRuleOutput]:
    try:
        # ✅ IMPORTANTE: sin HTML, todo literal
        ...
        if start == -1 or end == -1 or end <= start:
            raise ValueError("No JSON encontrado en la salida")

        json_text = raw_output[start:end+1]

        data = json.loads(json_text)

        # ✅ NORMALIZACIÓN (clave para que no falle Pydantic)
        data.setdefault("analysis", "")
        data.setdefault("propositions", [])
        data.setdefault("formula", "true")
        ...
        # ✅ FALLBACK → nunca rompe tu ejecución
        return ClinicalRuleOutput(
            analysis="fallback",
            propositions=[],
            formula="true"
        )
...
if __name__ == "__main__":
    ...
    for i, rule in enumerate(TEST_RULES, 1):
        ...
        prompt = build_compiler_prompt(rule)
